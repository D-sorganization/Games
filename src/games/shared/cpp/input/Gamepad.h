...
class Gamepad {
 public:
  ...
  void poll() {
    ...
    left_stick_.x =
        normalize_axis(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_LEFTX));
    left_stick_.y =
        normalize_axis(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_LEFTY));
    right_stick_.x =
        normalize_axis(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_RIGHTX));
    right_stick_.y =
        normalize_axis(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_RIGHTY));
    ...
    triggers_.left =
        normalize_trigger(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_TRIGGERLEFT));
    triggers_.right =
        normalize_trigger(SDL_GameControllerGetAxis(controller_, SDL_CONTROLLER_AXIS_TRIGGERRIGHT));

    // Buttons
    for (int i = 0; i < static_cast<int>(Button::Count); ++i) {
      bool prev = buttons_[i];
      buttons_[i] =
          SDL_GameControllerGetButton(controller_, static_cast<SDL_GameControllerButton>(i)) != 0;
  ...
  const StickState& left_stick() const noexcept {
    return left_stick_;
  }
  const StickState& right_stick() const noexcept {
    return right_stick_;
  }
  const TriggerState& triggers() const noexcept {
    return triggers_;
  }
  ...
  void set_deadzone(float dz) noexcept {
    deadzone_ = dz;
  }
