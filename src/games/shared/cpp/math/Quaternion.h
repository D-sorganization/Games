...
struct Quaternion {
  ...
  static Quaternion from_axis_angle(const Vec3& axis, float angle) {
    ...
    return {std::cos(half_angle), normalized_axis.x * s, normalized_axis.y * s,
            normalized_axis.z * s};
  }
  ...
  static Quaternion slerp(const Quaternion& a, const Quaternion& b, float t) {
    ...
    return {a.w * weight_a + b_adjusted.w * weight_b, a.x * weight_a + b_adjusted.x * weight_b,
            a.y * weight_a + b_adjusted.y * weight_b, a.z * weight_a + b_adjusted.z * weight_b};
  }
  ...
  static Quaternion nlerp(const Quaternion& a, const Quaternion& b, float t) {
    ...
    Quaternion result = {a.w + t * (b_adjusted.w - a.w), a.x + t * (b_adjusted.x - a.x),
                         a.y + t * (b_adjusted.y - a.y), a.z + t * (b_adjusted.z - a.z)};
  ...
  bool approx_equal(const Quaternion& other, float epsilon = 1e-5f) const noexcept {
    ...
    float d1 = std::abs(w - other.w) + std::abs(x - other.x) + std::abs(y - other.y) +
               std::abs(z - other.z);
    float d2 = std::abs(w + other.w) + std::abs(x + other.x) + std::abs(y + other.y) +
               std::abs(z + other.z);
