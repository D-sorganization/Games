...
class Shader {
 public:
  ...
  void set_vec3(const std::string& name, const math::Vec3& v) const {
    gl::glUniform3f(gl::glGetUniformLocation(program_id, name.c_str()), v.x, v.y, v.z);
  }
  ...
  void set_mat4(const std::string& name, const math::Mat4& m) const {
    gl::glUniformMatrix4fv(gl::glGetUniformLocation(program_id, name.c_str()), 1, GL_FALSE,
                           m.data());
  }
