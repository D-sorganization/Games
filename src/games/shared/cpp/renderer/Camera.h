...
class Camera {
 public:
  ...
  void toggle_mode() noexcept {
    set_mode(mode_ == CameraMode::FirstPerson ? CameraMode::ThirdPerson : CameraMode::FirstPerson);
  }
  ...
  void process_scroll(float delta) {
    if (mode_ == CameraMode::ThirdPerson) {
      ...
      config.orbit_distance =
          std::clamp(config.orbit_distance, config.orbit_min_dist, config.orbit_max_dist);
    }
  }

  /** Process keyboard movement with sprint support. */
  void process_movement(float forward, float right, float up, bool sprinting, float dt) {
  ...
  math::Mat4 projection_matrix() const {
    return math::Mat4::perspective(config.fov_y, config.aspect, config.near_z, config.far_z);
  }
  ...
  const math::Quaternion& orientation() const noexcept {
  ...
  void set_fov(float fov_radians) noexcept {
    config.fov_y = fov_radians;
  }

  /** Get the TPS target position (what the camera orbits around). */
  const math::Vec3& tps_target() const noexcept {
    return tps_target_;
  }
  ...
  float current_speed() const noexcept {
 ...
 private:
  ...
  void process_mouse_fps(float dx, float dy) {
    ...
    accumulated_pitch_ = std::clamp(accumulated_pitch_, -config.max_pitch, config.max_pitch);
    rebuild_fps_orientation();
  }

  void rebuild_fps_orientation() {
    yaw_quat_ = math::Quaternion::from_axis_angle(math::Vec3::up(), yaw_angle_);
  ...
  void update_fps(float dt) {
    if (config.smoothing > 0.0f) {
      ...
      current_orientation_ = math::Quaternion::slerp(current_orientation_, target_orientation_, t);
    } else {
      current_orientation_ = target_orientation_;
    }
  }
  ...
  void update_tps(float dt) {
    ...
    math::Vec3 orbit_offset = {sin_y * cos_p * config.orbit_distance,
    ...
    if (look_dir.length_squared() > 1e-6f) {
      ...
      current_orientation_ = math::Quaternion::slerp(current_orientation_, target_orient, orient_t);
    }
  }
};
