...
struct CombatStats {
  ...
  float accuracy() const {
    return total_shots > 0 ? static_cast<float>(total_hits) / total_shots * 100.0f : 0.0f;
  }
...
inline void shoot(const math::Vec3& origin, const math::Vec3& direction, const CombatConfig& config,
                  std::vector<core::Projectile>& projectiles, std::vector<core::Entity>& entities,
                  CombatStats& stats) {
  stats.total_shots++;
...
inline void check_projectile_collisions(std::vector<core::Projectile>& projectiles,
                                        std::vector<core::Entity>& entities, CombatStats& stats,
                                        int kill_score = 100) {
