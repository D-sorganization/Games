...
private:
    ...
    static void parse_visual(const std::string& visual, URDFLink& link) {
        ...
        if (!origin_tag.empty()) {
            ...
            if (!xyz.empty()) link.visual_origin.xyz = parse_vec3(xyz);
            if (!rpy.empty()) link.visual_origin.rpy = parse_vec3(rpy);
        }
    ...
    static void parse_geometry(const std::string& geom, URDFGeometry& g) {
        ...
        if (!box.empty()) {
            ...
            if (!size.empty()) g.size = parse_vec3(size);
            return;
        }
        ...
        if (!cyl.empty()) {
            ...
            if (!r.empty()) g.radius = std::stof(r);
            if (!l.empty()) g.length = std::stof(l);
            return;
        }
    ...
    static void parse_joints(const std::string& xml, URDFLoadResult& result) {
        ...
        for (const auto& block : joint_blocks) {
            ...
            if (!origin_tag.empty()) {
                std::string xyz = get_attr(origin_tag, "xyz");
                std::string rpy = get_attr(origin_tag, "rpy");
                if (!xyz.empty()) joint.origin.xyz = parse_vec3(xyz);
                if (!rpy.empty()) joint.origin.rpy = parse_vec3(rpy);
            }
            ...
            if (!axis_tag.empty()) {
                ...
                if (!axis_xyz.empty()) joint.axis = parse_vec3(axis_xyz);
            }
