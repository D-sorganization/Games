...
class Transform {
 public:
  ...
  Transform(const math::Vec3& pos, const math::Quaternion& rot, const math::Vec3& scl) noexcept
  ...
  const math::Quaternion& rotation() const noexcept {
    return rotation_;
  }
  const math::Vec3& scale() const noexcept {
    return scale_;
  }
   ...
   */
  void look_at(const math::Vec3& target, const math::Vec3& /*world_up*/ = math::Vec3::up()) {
  ...
  static Transform interpolate(const Transform& a, const Transform& b, float t) {
    return Transform(a.position_.lerp(b.position_, t),
