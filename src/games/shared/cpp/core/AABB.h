...
struct AABB {
   ...
   */
  bool ray_intersect(const math::Vec3& origin, const math::Vec3& dir, float& t_out) const {
    ...
    if (!slab(origin.x, dir.x, min.x, max.x))
      return false;
    if (!slab(origin.y, dir.y, min.y, max.y))
      return false;
    if (!slab(origin.z, dir.z, min.z, max.z))
      return false;
  ...
  AABB transformed(const math::Vec3& pos, const math::Vec3& scale) const {
    ...
    return {{std::min(scaled_min.x, scaled_max.x), std::min(scaled_min.y, scaled_max.y),
             std::min(scaled_min.z, scaled_max.z)},
            {std::max(scaled_min.x, scaled_max.x), std::max(scaled_min.y, scaled_max.y),
