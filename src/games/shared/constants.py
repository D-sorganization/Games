"""Shared constants for all FPS games in the Games framework.

This module contains the common gameplay constants shared across
Duum, Force Field, and Zombie Survival. Game-specific constants
(colors, themes, unique enemy types) remain in each game's own
constants module and can override these defaults.

Usage in game-specific constants.py::

    from games.shared.constants import GameState, SCREEN_WIDTH, ...
    # Then override only what differs, e.g.:
    # SKY_COLOR = (20, 0, 0)
"""

from __future__ import annotations

import math
from enum import Enum


# ---------------------------------------------------------------------------
# Game States
# ---------------------------------------------------------------------------
class GameState(Enum):
    """Game states shared across all FPS games."""

    INTRO = "intro"
    MENU = "menu"
    KEY_CONFIG = "key_config"
    MAP_SELECT = "map_select"
    PLAYING = "playing"
    LEVEL_COMPLETE = "level_complete"
    GAME_OVER = "game_over"


# ---------------------------------------------------------------------------
# Screen & Display
# ---------------------------------------------------------------------------
SCREEN_WIDTH = 1200
SCREEN_HEIGHT = 800
FPS = 60

# ---------------------------------------------------------------------------
# Rendering Quality
# ---------------------------------------------------------------------------
# 1 = Ultra (Full Res), 2 = High (Half Res),
# 4 = Medium/Retro (Quarter Res), 8 = Low (Blocky)
DEFAULT_RENDER_SCALE = 2
RENDER_SCALE_OPTIONS = [1, 2, 4, 8]
RENDER_SCALE_NAMES: dict[int, str] = {
    1: "ULTRA",
    2: "HIGH",
    4: "MEDIUM",
    8: "LOW",
}

# ---------------------------------------------------------------------------
# Map
# ---------------------------------------------------------------------------
MAP_SIZE = 40
TILE_SIZE = 64
MIN_BUILDING_OFFSET = 3
DEFAULT_MAP_SIZE = 40
MAP_SIZES = [20, 30, 40, 50, 60]

# ---------------------------------------------------------------------------
# Player
# ---------------------------------------------------------------------------
PLAYER_SPEED = 0.375
PLAYER_SPRINT_SPEED = 0.575
PLAYER_ROT_SPEED = 0.0015
SENSITIVITY_X = 1.0
MAX_RAYCAST_STEPS = 1000

FOV = math.pi / 3  # 60 degrees
HALF_FOV = FOV / 2
ZOOM_FOV_MULT = 0.5

MAX_DEPTH = 100
FOG_START = 0.4

DEFAULT_PLAYER_SPAWN = (2.5, 2.5, 0.0)
SPAWN_SAFE_ZONE_RADIUS = 15.0
SPAWN_SAFETY_MARGIN = 5

# ---------------------------------------------------------------------------
# New Game Defaults
# ---------------------------------------------------------------------------
DEFAULT_LIVES = 3
DEFAULT_DIFFICULTY = "NORMAL"
DEFAULT_START_LEVEL = 1

# ---------------------------------------------------------------------------
# Difficulty Settings
# ---------------------------------------------------------------------------
DIFFICULTIES: dict[str, dict[str, float]] = {
    "EASY": {"damage_mult": 0.5, "health_mult": 0.7, "score_mult": 0.5},
    "NORMAL": {"damage_mult": 1.0, "health_mult": 1.0, "score_mult": 1.0},
    "HARD": {"damage_mult": 1.5, "health_mult": 1.5, "score_mult": 2.0},
    "NIGHTMARE": {"damage_mult": 2.5, "health_mult": 2.0, "score_mult": 4.0},
}

# ---------------------------------------------------------------------------
# Aiming
# ---------------------------------------------------------------------------
SPREAD_BASE = 0.05
SPREAD_ZOOM = 0.005

# ---------------------------------------------------------------------------
# Weapon Ranges
# ---------------------------------------------------------------------------
WEAPON_RANGE_PISTOL = 15
WEAPON_RANGE_RIFLE = 25
WEAPON_RANGE_SHOTGUN = 12
WEAPON_RANGE_PLASMA = 30
WEAPON_RANGE_STORMTROOPER = 30
WEAPON_RANGE_MINIGUN = 20
WEAPON_RANGE_SNIPER = 40

# ---------------------------------------------------------------------------
# Common Colors
# ---------------------------------------------------------------------------
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0, 0, 255)
YELLOW = (255, 255, 0)
CYAN = (0, 255, 255)
GRAY = (128, 128, 128)
DARK_GRAY = (40, 40, 40)
PURPLE = (160, 32, 240)
BROWN = (100, 80, 60)
DARK_BROWN = (70, 50, 35)

# ---------------------------------------------------------------------------
# Fog
# ---------------------------------------------------------------------------
FOG_COLOR = DARK_GRAY

# ---------------------------------------------------------------------------
# Health & Combat
# ---------------------------------------------------------------------------
MAX_HEALTH = 100
INITIAL_HEALTH = 100
HEADSHOT_MULTIPLIER = 2.0
MELEE_RANGE = 3.0
BASE_MELEE_DAMAGE = 25

# ---------------------------------------------------------------------------
# Bot / Enemy
# ---------------------------------------------------------------------------
BOT_SAFE_SPAWN_RADIUS = 15.0
BOT_SPAWN_ATTEMPTS = 20
BOSS_SPAWN_ATTEMPTS = 50
MAX_ENEMIES_PER_LEVEL = 50
BASE_ENEMIES_PER_LEVEL = 5
ENEMIES_SCALE_PER_LEVEL = 2

# ---------------------------------------------------------------------------
# Pickups
# ---------------------------------------------------------------------------
PICKUP_SPAWN_CHANCE = 0.4
AMMO_BOMB_SPAWN_COUNT = 8
BOMB_CHANCE = 0.2
AMMO_CHANCE = 0.5

# ---------------------------------------------------------------------------
# Visual Effects
# ---------------------------------------------------------------------------
DAMAGE_TEXT_DURATION = 60
RESPAWN_MESSAGE_DURATION = 120
MESSAGE_VERTICAL_SPEED = -0.5


# ---------------------------------------------------------------------------
# Bot Animation
# ---------------------------------------------------------------------------
DEATH_ANIMATION_FRAMES = 60
DISINTEGRATE_FRAMES = 100

# ---------------------------------------------------------------------------
# Pickup / Portal Distances
# ---------------------------------------------------------------------------
PICKUP_RADIUS = 0.8
PORTAL_RADIUS = 1.5
PICKUP_RADIUS_SQ = PICKUP_RADIUS * PICKUP_RADIUS
PORTAL_RADIUS_SQ = PORTAL_RADIUS * PORTAL_RADIUS

# ---------------------------------------------------------------------------
# Combat
# ---------------------------------------------------------------------------
COMBO_TIMER_FRAMES = 180
MINIGUN_SPREAD = 0.15
MINIGUN_BULLETS_PER_SHOT = 3

# ---------------------------------------------------------------------------
# Fog
# ---------------------------------------------------------------------------
FOG_REVEAL_RADIUS = 5

# ---------------------------------------------------------------------------
# Atmosphere Timers
# ---------------------------------------------------------------------------
GROAN_TIMER_DELAY = 240
PAUSE_HEARTBEAT_DELAY = 51
BEAST_TIMER_MIN = 300
BEAST_TIMER_MAX = 900

# ---------------------------------------------------------------------------
# Music
# ---------------------------------------------------------------------------
MUSIC_TRACKS = [
    "music_loop",
    "music_drums",
    "music_wind",
    "music_horror",
    "music_piano",
    "music_action",
]


FPS_SHARED_CONSTANTS: dict[str, object] = {
    "SCREEN_WIDTH": SCREEN_WIDTH,
    "SCREEN_HEIGHT": SCREEN_HEIGHT,
    "FPS": FPS,
    "DEFAULT_RENDER_SCALE": DEFAULT_RENDER_SCALE,
    "RENDER_SCALE_OPTIONS": RENDER_SCALE_OPTIONS,
    "RENDER_SCALE_NAMES": RENDER_SCALE_NAMES,
    "MAP_SIZE": MAP_SIZE,
    "TILE_SIZE": TILE_SIZE,
    "MIN_BUILDING_OFFSET": MIN_BUILDING_OFFSET,
    "DEFAULT_MAP_SIZE": DEFAULT_MAP_SIZE,
    "MAP_SIZES": MAP_SIZES,
    "PLAYER_SPEED": PLAYER_SPEED,
    "PLAYER_SPRINT_SPEED": PLAYER_SPRINT_SPEED,
    "PLAYER_ROT_SPEED": PLAYER_ROT_SPEED,
    "SENSITIVITY_X": SENSITIVITY_X,
    "MAX_RAYCAST_STEPS": MAX_RAYCAST_STEPS,
    "FOV": FOV,
    "HALF_FOV": HALF_FOV,
    "ZOOM_FOV_MULT": ZOOM_FOV_MULT,
    "MAX_DEPTH": MAX_DEPTH,
    "FOG_START": FOG_START,
    "DEFAULT_PLAYER_SPAWN": DEFAULT_PLAYER_SPAWN,
    "SPAWN_SAFE_ZONE_RADIUS": SPAWN_SAFE_ZONE_RADIUS,
    "SPAWN_SAFETY_MARGIN": SPAWN_SAFETY_MARGIN,
    "DEFAULT_LIVES": DEFAULT_LIVES,
    "DEFAULT_DIFFICULTY": DEFAULT_DIFFICULTY,
    "DEFAULT_START_LEVEL": DEFAULT_START_LEVEL,
    "DIFFICULTIES": DIFFICULTIES,
    "WEAPON_RANGE_PISTOL": WEAPON_RANGE_PISTOL,
    "WEAPON_RANGE_RIFLE": WEAPON_RANGE_RIFLE,
    "WEAPON_RANGE_SHOTGUN": WEAPON_RANGE_SHOTGUN,
    "WEAPON_RANGE_PLASMA": WEAPON_RANGE_PLASMA,
    "WEAPON_RANGE_STORMTROOPER": WEAPON_RANGE_STORMTROOPER,
    "WEAPON_RANGE_MINIGUN": WEAPON_RANGE_MINIGUN,
    "WEAPON_RANGE_SNIPER": WEAPON_RANGE_SNIPER,
    "SPREAD_BASE": SPREAD_BASE,
    "SPREAD_ZOOM": SPREAD_ZOOM,
    "BLACK": BLACK,
    "WHITE": WHITE,
    "RED": RED,
    "GREEN": GREEN,
    "BLUE": BLUE,
    "YELLOW": YELLOW,
    "CYAN": CYAN,
    "GRAY": GRAY,
    "DARK_GRAY": DARK_GRAY,
    "PURPLE": PURPLE,
    "BROWN": BROWN,
    "DARK_BROWN": DARK_BROWN,
    "MAX_HEALTH": MAX_HEALTH,
    "INITIAL_HEALTH": INITIAL_HEALTH,
    "HEADSHOT_MULTIPLIER": HEADSHOT_MULTIPLIER,
    "MELEE_RANGE": MELEE_RANGE,
    "BASE_MELEE_DAMAGE": BASE_MELEE_DAMAGE,
    "BOT_SAFE_SPAWN_RADIUS": BOT_SAFE_SPAWN_RADIUS,
    "BOT_SPAWN_ATTEMPTS": BOT_SPAWN_ATTEMPTS,
    "BOSS_SPAWN_ATTEMPTS": BOSS_SPAWN_ATTEMPTS,
    "MAX_ENEMIES_PER_LEVEL": MAX_ENEMIES_PER_LEVEL,
    "BASE_ENEMIES_PER_LEVEL": BASE_ENEMIES_PER_LEVEL,
    "ENEMIES_SCALE_PER_LEVEL": ENEMIES_SCALE_PER_LEVEL,
    "PICKUP_SPAWN_CHANCE": PICKUP_SPAWN_CHANCE,
    "AMMO_BOMB_SPAWN_COUNT": AMMO_BOMB_SPAWN_COUNT,
    "BOMB_CHANCE": BOMB_CHANCE,
    "AMMO_CHANCE": AMMO_CHANCE,
    "DAMAGE_TEXT_DURATION": DAMAGE_TEXT_DURATION,
    "RESPAWN_MESSAGE_DURATION": RESPAWN_MESSAGE_DURATION,
    "MESSAGE_VERTICAL_SPEED": MESSAGE_VERTICAL_SPEED,
    "DEATH_ANIMATION_FRAMES": DEATH_ANIMATION_FRAMES,
    "DISINTEGRATE_FRAMES": DISINTEGRATE_FRAMES,
    "PICKUP_RADIUS": PICKUP_RADIUS,
    "PORTAL_RADIUS": PORTAL_RADIUS,
    "PICKUP_RADIUS_SQ": PICKUP_RADIUS_SQ,
    "PORTAL_RADIUS_SQ": PORTAL_RADIUS_SQ,
    "COMBO_TIMER_FRAMES": COMBO_TIMER_FRAMES,
    "MINIGUN_SPREAD": MINIGUN_SPREAD,
    "MINIGUN_BULLETS_PER_SHOT": MINIGUN_BULLETS_PER_SHOT,
    "FOG_REVEAL_RADIUS": FOG_REVEAL_RADIUS,
    "GROAN_TIMER_DELAY": GROAN_TIMER_DELAY,
    "PAUSE_HEARTBEAT_DELAY": PAUSE_HEARTBEAT_DELAY,
    "BEAST_TIMER_MIN": BEAST_TIMER_MIN,
    "BEAST_TIMER_MAX": BEAST_TIMER_MAX,
    "MUSIC_TRACKS": MUSIC_TRACKS,
    "JOYSTICK_DEADZONE": 0.1,
    "PITCH_LIMIT": 390,
    "SENSITIVITY_Y": 1.0,
    "SHIELD_MAX_DURATION": 600,
    "SHIELD_COOLDOWN_NORMAL": 600,
    "SHIELD_COOLDOWN_DEPLETED": 900,
    "BOMB_RADIUS": 10,
    "BOMB_COOLDOWN": 1800,
    "SAVE_FILE_PATH": "savegame.txt",
    "SPAWN_RETRY_RADIUS": 4,
    "BOT_SPEED": 0.02,
    "PLAYER_HEALTH": 100,
    "CHEAT_AMMO_AMOUNT": 999,
    "BASE_BOT_HEALTH": 30,
    "BASE_BOT_DAMAGE": 2,
    "BOT_ATTACK_RANGE": 5,
    "BOT_ATTACK_COOLDOWN": 60,
    "BOT_PROJECTILE_SPEED": 0.08,
    "BOT_PROJECTILE_DAMAGE": 5,
    "SECONDARY_COOLDOWN": 600,
    "SECONDARY_DAMAGE_MULT": 10.0,
    "PLASMA_AOE_RADIUS": 3.0,
    "PARTICLE_LIFETIME": 30,
    "INTRO_FADE_MAX": 255,
    "INTRO_FADE_SCALE": 510,
}
