...
class MinigunnerStyleRenderer(BaseBotStyleRenderer):
    ...
    def render(
        ...
        head_y = ry
        pygame.draw.rect(screen, (30, 30, 40), (int(head_x), int(head_y), int(head_size), int(head_size)))
        # Visor
        pygame.draw.rect(
            screen, (255, 0, 0), (int(head_x + 5), int(head_y + 10), int(head_size - 10), 5)
        )
        ...
        if getattr(bot, "shoot_animation", 0) > 0:
            pygame.draw.circle(
                screen, (255, 255, 0), (int(wx), int(wy + weapon_h / 2)), 5 + random.randint(0, 5)
            )
