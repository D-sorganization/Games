...
def render_menu(renderer: UIRenderer) -> None:
    """Render the Duum main menu."""
    renderer.screen.fill(C.BLACK)

    title = renderer.title_font.render("DUUM", True, C.RED)
    title_rect = title.get_rect(center=(C.SCREEN_WIDTH // 2, 100))

    subtitle = renderer.subtitle_font.render("KNEE DEEP IN THE CODE", True, C.WHITE)
...
def render_map_select(renderer: UIRenderer, game: Game) -> None:
    """Render the Duum map-selection screen."""
    renderer.screen.fill(C.BLACK)
