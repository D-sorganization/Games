...
Recent maintenance keeps CI and developer hooks fail-closed on Windows and self-hosted runners: the anti-phantom guard uses `gh pr view --json files` for compatibility with older GitHub CLI builds, Python formatting is enforced through Black as the single formatter, and CI tool installation tolerates corrupted shared runner packages by reinstalling required versions explicitly.
...
The canonical architecture map, system context, container boundaries, and feature traceability are maintained in [`docs/architecture/C4.md`](docs/architecture/C4.md) and enforced by `scripts/architecture_map_contract.py` per Epic #1594.
...
- [x] Unit test: Workflow inline Python contains no bare `except:`, and the PR-Comment-Responder queue load distinguishes a missing queue from a corrupt one, reporting the latter
...
| 2026-08-29 | 1.1.38  | Follow-up to `#854`: added `PR-Comment-Responder.yml` to the `cancel-in-progress` exception allowlist in `lint-workflow-files.yml`. That workflow deliberately sets `cancel-in-progress: false` because it read-modify-writes a shared queue file, so any PR touching it failed the workflow-lint gate; setting the flag to `true` instead would reintroduce exactly the truncation #854 fixed. Pinned the exemption and the setting together with a test so neither half can drift.                                                                                                                                                                                       |
| 2026-08-29 | 1.1.37  | Addresses issue `#854`: replaced the bare `except:` guarding the PR-Comment-Responder queue load, which turned any read failure (truncated file, permission error, full disk) into a silent reset that overwrote the real backlog with an empty queue. `FileNotFoundError` still starts an empty queue; `json.JSONDecodeError` now emits a `::warning::` naming the file before resetting, so the loss is visible in the run output. Added `tests/test_workflow_python_hygiene.py`, a static contract over the inline Python in every workflow -- which the runtime lint gates never see -- banning bare `except:` repo-wide and pinning the queue-load handler structure. |
...
| 2026-04-14 | 1.1.28  | Raised statement coverage toward 80% by adding 8 test files (126+ tests): Zombie Survival atmosphere_manager, weapon_system, screen_event_handler, renderer, and game delegation; Duum renderer and screen_event_handler extensions; Force Field combat_actions. Addresses issue #738.                                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-04-14 | 1.1.26  | Removed tracked coverage artifact directories (`cov_test3/`, `cov_annot2/`, `cov.json`) from git history and added coverage output patterns to `.gitignore` to prevent future accidental commits. Addresses issue #766.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-04-14 | 1.1.25  | Extracted `raycaster_textures` module from the monolithic `raycaster.py` to reduce file size and improve separation of concerns; decomposed oversized rendering functions into focused single-responsibility helpers. Addresses issue #741.                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- 2026-09-14: Downgraded non-existent workflow action versions to @v4/@v5 across workflows (#863).
