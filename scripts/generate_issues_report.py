...
def main():
    ...
    for cat_id, details in category_scores.items():
        ...
        if score < 5:
            issues.append(f"- Category {cat_id} ({details.get('name')}) score {score} < 5. Label: jules:assessment,needs-attention")
    ...
    with open(output_path, "w") as f:
        if issues:
            ...
            f.write("\n".join(issues))
            f.write("\n")
            print(f"Found {len(issues)} categories with score < 5. Report saved to {output_path}")
        else:
