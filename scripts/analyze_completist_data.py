...
def generate_mermaid_charts(
    ...
    # Breakdown by Top Modules (Bar Chart equivalent using pie or just text for now as
    # mermaid bar is verbose)
...
def generate_report() -> None:
    ...
    report = [
        ...
        f"- **Feature Gaps (TO" f"DO)**: {len(todos)}",
    ...
    for item in criticals[:50]:
        ...
        report.append(
            f"| `{item['file']}` | {item['line']} | {item['type']} | {imp} | "
            f"{cov} | {comp} |"
        )
