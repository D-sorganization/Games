...
def _mock_weapon_renderer(monkeypatch):
    ...
    monkeypatch.setattr("games.Duum.src.renderer.WeaponRenderer", mock_wr)
    return mock_wr


def _make_renderer():
    from games.Duum.src.renderer import GameRenderer
...
def _make_game(**overrides):
    ...
    base["particle_system"].world_particles = []
    base.update(overrides)
    return SimpleNamespace(**base)


# ---------------------------------------------------------------------------
# render_game — DbC (Duum takes flash_intensity param)
# ---------------------------------------------------------------------------
...
def test_render_game_calls_raycaster_render_3d_with_flash_intensity() -> None:
    renderer = _make_renderer()
    game = _make_game()
    renderer.render_game(game, 0.5)
    call_kwargs = game.raycaster.render_3d.call_args
    assert call_kwargs.kwargs.get("flash_intensity") == 0.5
...
# ---------------------------------------------------------------------------
# _render_particles — laser, trace, normal
# ---------------------------------------------------------------------------
...
def test_render_particles_trace_type(monkeypatch) -> None:
    import pygame
