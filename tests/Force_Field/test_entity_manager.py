"""Tests for games.Force_Field.src.entity_manager."""
...
class TestFFEntityManagerInit:
    def test_init_empty(self, em: EntityManager) -> None:
        assert em.bots == []
        assert em.projectiles == []

    def test_reset(self, em: EntityManager, mock_bot: MagicMock) -> None:
...
class TestFFEntityManagerAddRemove:
...
class TestFFEntityManagerGetActive:
    ...
    def test_get_nearest_enemy_distance_no_bots(self, em: EntityManager) -> None:
        import math

        dist = em.get_nearest_enemy_distance(5.0, 5.0)
        assert dist == math.inf

    def test_get_nearest_enemy_distance_with_bot(
...
class TestFFEntityManagerUpdateBots:
...
class TestFFEntityManagerUpdateProjectiles:
    ...
    def test_enemy_proj_misses_player(
        self,
        em: EntityManager,
        mock_player: MagicMock,
        mock_map: MagicMock,
    ...
    def test_plasma_explosion_on_wall_hit(
        self,
        em: EntityManager,
        mock_player: MagicMock,
        mock_map: MagicMock,
        ...
        em.projectiles = [proj]
        em.update_projectiles(mock_map, mock_player, mock_game)
        mock_game.combat_system.explode_plasma.assert_called_once_with(proj)

    def test_rocket_explosion_on_wall_hit(
        self,
        em: EntityManager,
        mock_player: MagicMock,
        mock_map: MagicMock,
        ...
        em.projectiles = [proj]
        em.update_projectiles(mock_map, mock_player, mock_game)
        mock_game.combat_system.explode_rocket.assert_called_once_with(proj)

    def test_bomb_explosion_on_wall_hit(
        self,
        em: EntityManager,
        mock_player: MagicMock,
        mock_map: MagicMock,
        ...
        em.projectiles = [proj]
        em.update_projectiles(mock_map, mock_player, mock_game)
        mock_game.combat_system.explode_bomb.assert_called_once_with(proj)

    def test_dead_projectiles_cleaned_up(
        self,
        em: EntityManager,
        mock_player: MagicMock,
        mock_map: MagicMock,
    ...
    def test_player_proj_plasma_on_bot_hit(
        self,
        em: EntityManager,
        mock_bot: MagicMock,
        mock_player: MagicMock,
        ...
        em.spatial_grid.get_nearby = MagicMock(return_value=[mock_bot])
        em.update_projectiles(mock_map, mock_player, mock_game)
        mock_game.combat_system.explode_plasma.assert_called()
