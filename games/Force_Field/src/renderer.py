        ...
        if portal:
            ...
            if b > 0.1:
                ...
                if -base_size < screen_x < C.SCREEN_WIDTH + base_size:
                    ...
                    rings = [
                        {"size": base_size * 0.8 * pulse1, "color": (0, 255, 255), "width": 8},
                        {"size": base_size * 0.6 * pulse2, "color": (100, 255, 255), "width": 6},
                        {"size": base_size * 0.4 * pulse1, "color": (200, 255, 255), "width": 4},
                        {"size": base_size * 0.2 * pulse2, "color": (255, 255, 255), "width": 3},
                    ]
                    
                    # Draw outer glow effect
                    glow_surface = pygame.Surface((base_size * 2, base_size * 2), pygame.SRCALPHA)
                    glow_color = (0, 200, 255, 30)
                    pygame.draw.circle(glow_surface, glow_color, 
                                     (base_size, base_size), int(base_size * 0.9))
                    self.screen.blit(glow_surface, 
                                   (screen_x - base_size, screen_y - base_size))
                    ...
                    for ring in rings:
                        ...
                        if size > 0:
                            # Main ring
                            pygame.draw.circle(self.screen, ring["color"], center, 
                                             size, ring["width"])
                            
                            # Inner glow
                            inner_color = tuple(min(255, c + 50) for c in ring["color"])
                            pygame.draw.circle(self.screen, inner_color, center, 
                                             size - ring["width"]//2, 2)
