...
class BotRenderer:
    ...
    def _render_ball(
        ...
        for ring in range(3):
            ...
            for i in range(12):
                ...
                point_y = cy + math.sin(point_angle) * ring_radius * 0.3  # Flatten for 3D effect
    ...
    def _render_beast(
        ...
        body_rect = (cx - current_rw / 2, ry + current_rh * 0.25, current_rw, current_rh * 0.6)
        pygame.draw.rect(screen, beast_color, body_rect, border_radius=int(current_rw * 0.1))
        
        # Body armor plating
        for i in range(3):
            plate_y = ry + current_rh * (0.3 + i * 0.15)
            plate_rect = (
                cx - current_rw * 0.4, plate_y, 
                current_rw * 0.8, current_rh * 0.08
            )
            pygame.draw.rect(screen, dark_beast, plate_rect, border_radius=2)
            # Highlight on armor
            highlight_rect = (
                cx - current_rw * 0.35, plate_y + 2, 
                current_rw * 0.7, current_rh * 0.04
            )
        ...
        pygame.draw.rect(screen, (120, 0, 0), head_rect, border_radius=int(head_size * 0.1))
        ...
        for eye_offset in [-0.25, 0.25]:
            ...
            # Eye glow
            eye_glow_surf = pygame.Surface((eye_glow_size * 4, eye_glow_size * 4), pygame.SRCALPHA)
            pygame.draw.circle(eye_glow_surf, (255, 255, 0, 80), 
                             (eye_glow_size * 2, eye_glow_size * 2), int(eye_glow_size * 2))
            screen.blit(eye_glow_surf, (eye_x - eye_glow_size * 2, eye_y - eye_glow_size * 2))
            
            # Main eye
            pygame.draw.circle(screen, (255, eye_brightness, 0), 
                             (int(eye_x), int(eye_y)), int(eye_glow_size))
            # Eye pupil
            pygame.draw.circle(screen, (255, 255, 255), 
                             (int(eye_x), int(eye_y)), int(eye_glow_size * 0.4))
        ...
        if not bot.dead:
            ...
            pygame.draw.line(screen, beast_color, left_shoulder, left_hand, arm_thickness)
            ...
            pygame.draw.line(screen, beast_color, right_shoulder, right_hand, arm_thickness)
        ...
        if not bot.dead:
            ...
            pygame.draw.rect(screen, dark_beast, 
            ...
            for foot_x in [cx - current_rw * 0.15, cx + current_rw * 0.2]:
                ...
                pygame.draw.ellipse(screen, (40, 40, 40), 
                                  (foot_x - 8, foot_y - 3, 16, 8))
                # Foot claws
                for toe in range(3):
                    toe_x = foot_x - 6 + toe * 6
                    pygame.draw.line(screen, (180, 180, 180),
                                   (toe_x, foot_y), (toe_x, foot_y + 8), 2)
    ...
    def _render_ghost(
        ...
        head_radius = rw / 2
        head_center = (int(cx), int(gy + head_radius))
        ...
        pygame.draw.circle(ghost_surface, ghost_color, 
                         (int(rw), int(head_radius)), int(head_radius))
        
        # Head highlight for ethereal effect
        highlight_color = (*color, int(base_alpha * 0.6))
        pygame.draw.circle(ghost_surface, highlight_color,
        ...
        body_width = rw
        body_height = rh * 0.7
        body_rect = pygame.Rect(rw - body_width / 2, head_radius, body_width, body_height)
        ...
        for i in range(tatter_count + 1):
            ...
            y = head_radius + body_height + base_tatter_length + wave_offset + length_variation
        ...
        for eye_x_offset in [-0.25, 0.25]:
            ...
            pygame.draw.circle(eye_glow_surf, (0, 255, 255, glow_brightness), 
                             (eye_glow_size, eye_glow_size), eye_glow_size)
            ghost_surface.blit(eye_glow_surf, (eye_x - eye_glow_size, eye_y - eye_glow_size))
            ...
            pygame.draw.circle(ghost_surface, pupil_color, (eye_x, eye_y), int(eye_size * 0.6))
        ...
        if random.random() < 0.1:  # 10% chance per frame
            for _ in range(3):
                ...
                pygame.draw.circle(ghost_surface, (200, 200, 255, breath_alpha), 
                                 (breath_x, breath_y), breath_size)
        ...
        if random.random() < 0.2:  # 20% chance per frame
            for _ in range(2):
                ...
                particle_y = gy + rw / 2 + math.sin(particle_angle) * particle_distance * 0.5
                
                particle_surface = pygame.Surface((6, 6), pygame.SRCALPHA)
                particle_alpha = random.randint(40, 120)
                pygame.draw.circle(particle_surface, (150, 150, 255, particle_alpha), (3, 3), 3)
    ...
    def _render_minigunner(
        ...
        pygame.draw.rect(screen, armor_color, main_armor, border_radius=int(current_rw * 0.1))
        ...
        pygame.draw.circle(screen, (255, 100, 100), (core_x, core_y), int(core_size * 0.6))
        ...
        pygame.draw.rect(screen, armor_shadow, helmet_rect, border_radius=int(head_size * 0.1))
        
        # Helmet highlight
        highlight_rect = (head_x + 3, head_y + 3, head_size - 6, head_size * 0.4)
        pygame.draw.rect(screen, armor_color, highlight_rect, border_radius=int(head_size * 0.08))
        ...
        for scan_line in range(3):
            ...
            pygame.draw.line(screen, (line_brightness, 0, 0),
                           (visor_x + 2, line_y), (visor_x + visor_width - 2, line_y), 1)
        ...
        weapon_scale = 1.0 + (bot.shoot_animation * 0.1 if bot.shoot_animation > 0 else 0)
        ...
        pygame.draw.rect(screen, (50, 50, 50), 
                        (wx + 5, wy + 3, weapon_w - 10, weapon_h - 6), border_radius=2)
        ...
        pygame.draw.circle(screen, (30, 30, 30), 
                         (int(barrel_center_x), int(barrel_center_y)), int(barrel_radius * 0.2))
        ...
        if bot.shoot_animation > 0:
            ...
            for i, flash_color in enumerate(flash_colors):
                ...
                if current_size > 0:
                    flash_surface = pygame.Surface((current_size * 2, current_size * 2), pygame.SRCALPHA)
                    pygame.draw.circle(flash_surface, flash_color, 
                                     (current_size, current_size), current_size)
                    screen.blit(flash_surface, (flash_x - current_size, flash_y - current_size))
            ...
            for _ in range(8):
                ...
                spark_size = random.randint(1, 3)
                pygame.draw.circle(screen, (255, 255, 100), (spark_x, spark_y), spark_size)
        ...
        if not bot.dead:
            ...
            pygame.draw.rect(screen, armor_color, 
                           (left_leg_x, leg_y, leg_w, leg_h), border_radius=4)
            ...
            pygame.draw.rect(screen, armor_color, 
                           (right_leg_x, leg_y, leg_w, leg_h), border_radius=4)
            ...
            for boot_x in [left_leg_x, right_leg_x]:
                ...
                pygame.draw.rect(screen, (60, 60, 60), 
                               (boot_x, boot_y + 2, leg_w, 8), border_radius=1)
    ...
    def _render_monster(
        ...
        if not bot.dead or bot.death_timer < 30:
            ...
            pygame.draw.rect(screen, base_color, head_rect, border_radius=head_size // 6)
            
            # Head highlight
            highlight_head = pygame.Rect(
                cx - head_size // 3, head_y + 2, 
                head_size // 1.5, head_size // 3
            )
            pygame.draw.rect(screen, light_color, highlight_head, border_radius=head_size // 8)
            ...
            for eye_x_offset in [-0.25, 0.25]:
                ...
                # Outer glow
                glow_surf = pygame.Surface((eye_glow_size * 4, eye_glow_size * 4), pygame.SRCALPHA)
                pygame.draw.circle(glow_surf, (*eye_color, 60), 
                                 (eye_glow_size * 2, eye_glow_size * 2), int(eye_glow_size * 2))
                screen.blit(glow_surf, (eye_x - eye_glow_size * 2, eye_y - eye_glow_size * 2))
                
                # Main eye
                pygame.draw.circle(screen, eye_color, (int(eye_x), int(eye_y)), int(head_size * 0.08))
                # Eye pupil
                pygame.draw.circle(screen, (255, 255, 255), (int(eye_x), int(eye_y)), int(head_size * 0.03))
            ...
            if bot.mouth_open:
                ...
                inner_mouth = (cx - mouth_w / 2 + 3, mouth_y + 3, mouth_w - 6, head_size * 0.25 - 6)
                ...
                pygame.draw.line(
                    ...
                    (cx + mouth_w / 2, mouth_y + 3), 4
                )
                # Mouth shadow
                pygame.draw.line(
                    screen, dark_color,
                    (cx - mouth_w / 2, mouth_y + 6),
                    (cx + mouth_w / 2, mouth_y + 6), 2
                )
        ...
        if not bot.dead:
            ...
            # Upper arm
            pygame.draw.line(screen, base_color, shoulder_left, elbow_left, arm_thickness)
            # Lower arm
            pygame.draw.line(screen, base_color, elbow_left, hand_left, arm_thickness - 2)
            ...
            pygame.draw.line(screen, base_color, shoulder_right, elbow_right, arm_thickness)
            # Lower arm
            pygame.draw.line(screen, base_color, elbow_right, weapon_pos, arm_thickness - 2)
            ...
            pygame.draw.rect(screen, (80, 80, 80), 
                           (weapon_pos[0] - 3, weapon_pos[1] - 3, 26, 8), border_radius=1)
                ...
                for i, flash_color in enumerate(flash_colors):
                    ...
                    if current_size > 0:
                        pygame.draw.circle(screen, flash_color,
                                         (weapon_pos[0] + 30, weapon_pos[1]), current_size)
        ...
        if not bot.dead:
            ...
            pygame.draw.rect(screen, base_color, 
                           (left_leg_x, leg_y, leg_w, leg_h * 0.6), border_radius=3)
            pygame.draw.rect(screen, mid_color, 
                           (left_leg_x, leg_y + leg_h * 0.5, leg_w, leg_h * 0.5), border_radius=2)
            
            # Right leg
            right_leg_x = current_body_x + current_rw * 0.55
            pygame.draw.rect(screen, base_color, 
                           (right_leg_x, leg_y, leg_w, leg_h * 0.6), border_radius=3)
            pygame.draw.rect(screen, mid_color, 
                           (right_leg_x, leg_y + leg_h * 0.5, leg_w, leg_h * 0.5), border_radius=2)
            
            # Feet
            pygame.draw.ellipse(screen, (60, 60, 60), 
                              (left_leg_x - 2, leg_y + leg_h - 5, leg_w + 8, 8))
            pygame.draw.ellipse(screen, (60, 60, 60), 
                              (right_leg_x - 2, leg_y + leg_h - 5, leg_w + 8, 8))
